...
- **Baseline vs. Other Models:** The Dummy Regressor provides a weak baseline, with test $R^2$ values close to or below zero. In contrast, all machine learning models substantially ***improve predictive performance***, achieving high $R^2$ values for most weather parameters.

- **Bagging vs. Boosting Models:** ***Boosting generally outperforms*** Random Forest, although performance varies across targets and forecast horizons. ***XGBoost performs particularly well overall***, while XGBoost and LGBM are competitive for wind-related variables.

- **Best Model vs. AutoML Models:** The manually selected boosting models achieve performance comparable to or better than AutoML across most targets and horizons. ***XGBoost*** therefore provides a strong alternative to the automated solution while offering a ***specific and reproducible model configuration***.

- **Forecast Horizon Degradation:** Predictive performance generally decreases as the forecast horizon increases, with higher MAE and RMSE and lower $R^2$. XGBoost ***shows relatively stable performance***, indicating greater robustness to increasing forecast horizons.
